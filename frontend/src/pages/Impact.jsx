...
const HistoryIcon = () => (<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-5 h-5"><circle cx="12" cy="12" r="10"></circle><polyline points="12 6 12 12 16 14"></polyline></svg>);
...
// NEW: Results View Component
const ResultsView = ({ onCalculateAgain }) => {
    // Mock data for the results
    const resultData = {
        total: 51.0,
        breakdown: [
            { category: 'Transportation', value: 0.0, icon: '🚗' },
            { category: 'Energy', value: 0.0, icon: '⚡️' },
            { category: 'Food', value: 51.0, icon: '🥗' },
        ],
        globalAverage: 4000,
    };
    ...
    return (
        <div className="w-full max-w-md mx-auto font-sans mb-10 animate-fade-in-up">
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-xl font-bold text-gray-900">Your Results</h1>
                <button onClick={onCalculateAgain} className="flex items-center gap-2 text-sm font-semibold text-gray-700 bg-gray-100 hover:bg-gray-200 px-3 py-2 rounded-full">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4"><path d="M21.5 2v6h-6M2.5 22v-6h6"/><path d="M22 11.5A10 10 0 0 0 3.5 12.5"/><path d="M2 12.5a10 10 0 0 0 18.5-1"/></svg>
                    Calculate Again
                </button>
            </div>
            ...
            <div className="space-y-4">
                ...
                <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-md text-center space-y-3">
                    <div className="flex items-center justify-center gap-2"><LeafIcon /><h2 className="text-lg font-semibold text-gray-900">Your Carbon Footprint</h2></div>
                    <div>
                        <AnimatedNumber value={resultData.total} />
                        <span className="text-2xl text-gray-600"> kg CO₂</span>
                        <p className="text-gray-500">monthly emissions</p>
                    </div>
                    <p className="text-gray-600 font-medium">{ (resultData.total * 12).toFixed(1) } kg CO₂ annually</p>
                    <span className="inline-flex items-center gap-1 text-sm font-semibold bg-green-100 text-green-700 px-3 py-1 rounded-full">🌱 Low Impact</span>
                    <div className="text-left pt-4">
                        <div className="flex justify-between text-sm font-medium text-gray-600 mb-1"><span>vs Global Average</span><span>{resultData.globalAverage.toLocaleString()} kg CO₂/year</span></div>
                        <div className="w-full bg-gray-200 rounded-full h-2.5"><div className="bg-gray-800 h-2.5 rounded-full" style={{ width: `${(userAnnual / resultData.globalAverage) * 100}%` }}></div></div>
                        <p className="text-center text-sm text-gray-600 mt-2 font-medium">{percentBelowAverage.toFixed(0)}% below average</p>
                    </div>
                </div>
                ...
                {/* Breakdown Card */}
                <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-md space-y-4"><h3 className="font-semibold text-gray-900">Emissions Breakdown</h3>
                    {resultData.breakdown.map(item => (
                        <div key={item.category}>
                            <div className="flex justify-between items-center text-sm font-medium text-gray-700 mb-1"><span>{item.icon} {item.category}</span><span>{item.value.toFixed(1)} kg CO₂</span></div>
                            <div className="w-full bg-gray-200 rounded-full h-2"><div className="bg-emerald-500 h-2 rounded-full" style={{ width: `${(item.value / resultData.total) * 100}%` }}></div></div>
                        </div>
                    ))}
                </div>

                {/* Tips Card */}
                <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-md"><h3 className="font-semibold text-gray-900 mb-3">Ways to Reduce Your Impact</h3>
                    <ul className="list-disc list-inside space-y-2 text-emerald-700">
                        <li><span className="text-gray-700">Use public transport, bike, or walk instead of driving</span></li>
                        <li><span className="text-gray-700">Switch to renewable energy sources</span></li>
                        <li><span className="text-gray-700">Reduce meat consumption and food waste</span></li>
                        <li><span className="text-gray-700">Donate surplus food through Annapurna to prevent waste</span></li>
                    </ul>
                </div>
            </div>
        </div>
...
// Main View for Calculator Flow
const CalculatorView = ({ onBackClick }) => {
    const [mainView, setMainView] = useState('Calculator');
    const [calculatorState, setCalculatorState] = useState('form'); // 'form' or 'result'
    ...
    return (
        <div className="w-full max-w-md mx-auto font-sans mb-10">
            {calculatorState === 'form' ? (
                <>
                    {/* Header & Main Tabs only show for form/history, not results */}
                    <div className="flex items-center mb-6"><button onClick={onBackClick} className="p-2 -ml-2"><BackIcon /></button><div className="ml-2"><h1 className="text-xl font-bold text-gray-900">Carbon Footprint</h1><p className="text-sm text-gray-600">Calculate and track your environmental impact</p></div></div>
                    ...
                    {mainView === 'Calculator' && (
                        <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-md flex flex-col gap-6 animate-fade-in-up">
                            <div className="flex items-center gap-3"><CalculatorIcon /><h2 className="text-lg font-semibold text-gray-900">Carbon Footprint Calculator</h2></div>
                            <div><label htmlFor="time-period" className="block text-sm font-medium text-gray-700 mb-2">Time Period</label><select id="time-period" className="w-full bg-gray-100 border border-gray-200 rounded-lg p-3 text-gray-800 appearance-none focus:outline-none focus:ring-2 focus:ring-gray-400">
...
// --- END: CALCULATOR COMPONENTS ---

const AnimatedImpactStat = ({ icon, value: endValue, label, bgColor, unit }) => { const [currentValue, setCurrentValue] = useState(0); useEffect(() => { let s=null; const a = t => { if (!s) s=t; const p=Math.min((t - s)/1500,1); setCurrentValue(Math.floor(p*endValue)); if (p<1) requestAnimationFrame(a); }; requestAnimationFrame(a); }, [endValue]); return (<div className="bg-white p-4 rounded-2xl shadow-sm flex flex-col items-center justify-center text-center"><div className={`rounded-full p-3 mb-2 ${bgColor}`}>{icon}</div><span className="text-2xl font-bold text-gray-800">{currentValue}{unit || ''}</span><span className="text-sm text-gray-500">{label}</span></div>);};
  ...
  return (
    <div className="bg-gray-50 min-h-screen font-sans">
      <div className="container mx-auto lg:mx-0 p-4 max-w-md lg:max-w-screen lg:grid lg:grid-cols-1 mb-20">
        <div>
          <div className="bg-emerald-600 text-white p-5 rounded-2xl shadow-lg mb-6"><h1 className="text-2xl font-bold mb-4">Your Impact</h1><div className="flex items-center"><div className="bg-yellow-400 p-2 rounded-full mr-4"><FaStar /></div><div className='text-left'><p className="font-bold">Level {impactData.level} Donor</p><p className="text-sm">{impactData.points} points</p></div><div className="ml-auto text-right"><p className="font-bold">{impactData.totalDonations}</p><p className="text-sm">Total Donations</p></div></div><div className="mt-4"><p className="text-sm mb-1">Progress to Level {impactData.level + 1}</p><div className="w-full bg-emerald-300 rounded-full h-2.5"><div className="bg-white h-2.5 rounded-full" style={{ width: `${impactData.progressToNextLevel}%` }}></div></div><p className="text-right text-xs mt-1">{impactData.progressToNextLevel}/100</p></div></div>
          {showCalculator ? <CalculatorView onBackClick={handleBackClick} /> : <PromoCard onCalculateClick={handleCalculateClick} />}
          <div className="grid grid-cols-2 gap-4 mb-6"><AnimatedImpactStat icon={<FaLeaf />} value={impactData.co2Saved} unit=" kg" label="CO₂ Saved" bgColor="bg-green-100" /><AnimatedImpactStat icon={<FaRegSmile />} value={impactData.smilesShared} label="Smiles Shared" bgColor="bg-yellow-100" /><AnimatedImpactStat icon={<FaUsers />} value={impactData.peopleHelped} label="People Helped" bgColor="bg-blue-100" /><AnimatedImpactStat icon={<FaTrophy />} value={impactData.dayStreak} label="Day Streak" bgColor="bg-orange-100" /></div>
        </div>
